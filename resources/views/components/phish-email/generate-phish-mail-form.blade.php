<div>
    ...
    <div id="temp-suggestions" style="display: none;">
        <div >
            <p class="text-muted">{{ __('Suggestions:') }}</p>
        </div>
        <div class="row">
            <div class="col-lg-6">
                <button type="button" class="list-group-item text-muted list-group-item-action mb-3 border rounded py-2" aria-current="true">
                    ...
                    <em>{{ __('Generate an email template of dropbox password reset') }}</em>
                </button>
                <button type="button" class="list-group-item text-muted list-group-item-action mb-3 border rounded py-2">
                    ...
                    <em>{{ __('Generate an email template for microsoft account verification') }}</em>
                </button>
                <button type="button" class="list-group-item text-muted list-group-item-action mb-3 border rounded py-2">
                    <span class="avatar avatar-xs me-2 avatar-rounded">
                    ...
                    <em>{{ __('Generate an email template for google password reset') }}</em>
                </button>
            </div>
            <div class="col-lg-6">
                <button type="button" class="list-group-item text-muted list-group-item-action mb-3 border rounded py-2">
                    ...
                    <em>{{ __('Generate an email template for netflix subscription confirmation') }}</em>
                </button>
                <button type="button" class="list-group-item text-muted list-group-item-action mb-3 border rounded py-2">
                    <span class="avatar avatar-xs me-2 avatar-rounded">
                        <img src="https://cdn-icons-png.flaticon.com/512/871/871976.png" alt="img">
                    </span>
                    <em>{{ __('Generate an email template for event invitation') }}</em>
                </button>
                <button type="button" class="list-group-item text-muted list-group-item-action mb-3 border rounded py-2">
                    <span class="avatar avatar-xs me-2 avatar-rounded">
                        <img src="https://static-00.iconduck.com/assets.00/amazon-icon-2048x1722-myhuicq8.png" alt="img">
                    </span>
                    <em>{{ __('Generate an email template for amazon order confirmation') }}</em>
                </button>
            </div>
        </div>
    ...
    <div id="aiTempContainer" style="display: none;">
        <div>
            <p class="text-muted">{{ __('Please add the shortcodes') }} <code>@{{user_name}}</code>{{ __('(optional),') }}
                <code>@{{website_url}}</code> i.e. href="@{{website_url}}" {{ __('and') }}
                <code>@{{tracker_img}}</code> {{ __('anywhere') }}
            </p>
        </div>
        <div>
        ...
        <div class="d-flex justify-content-end mt-3">
            <button class="btn btn-primary" onclick="showAnotherModal(this)" type="button">{{ __('Save template') }}</button>
