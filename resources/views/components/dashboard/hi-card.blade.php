<div class="card custom-card projects-tracking-card overflow-hidden text-center">
    <div class="card-body">
        ...
        <div>
            <span class="fs-15 fw-semibold d-block mt-4">{{ __('Hi') }}, {{ Auth::user()->full_name }}</span>
            <span class="d-block fs-11 text-muted mt-2">{{ Auth::user()->company_name }}</span>
            <a href="{{ route('campaigns') }}" class="btn btn-primary btn-sm btn-wave waves-effect waves-light mt-3">{{ __('Start Simulation') }}</a>
        </div>
