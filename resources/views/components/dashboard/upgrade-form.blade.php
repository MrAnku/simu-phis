<form action="{{ route('reqNewLimit') }}" method="post">
    ...
    <div class="mb-3">
        <label for="input-label" class="form-label">{{ __('Current Limit') }}</label>
    ...
    <div class="mb-3">
        <label for="input-label" class="form-label">{{ __('New Limit') }}<sup class="text-danger">*</sup></label>
        <input type="number" name="new_limit" class="form-control" min="10" max="5000"
            placeholder="{{ __('Enter new limit value ex. 1000') }}">

    </div>
    <div class="mb-3">
        <label for="input-label" class="form-label">{{ __('Additional Information') }}</label>
