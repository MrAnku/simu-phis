...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('badges', function (Blueprint $table) {
            ...
            $table->string('criteria_type'); // score, courses_completed,etc.
            $table->string('criteria_operator')->default('>=');
        ...
        // For training assigned users table
        $allTrainingUsers = TrainingAssignedUser::all();
        foreach ($allTrainingUsers as $user) {
        ...
        // For scorm assigned users table
         $allScormUsers = ScormAssignedUser::all();
        foreach ($allScormUsers as $user) {
            $badge = getMatchingBadge('score', $user->personal_best);
            // This helper function accepts a criteria type and value, and returns the first matching badge
            ...
            $totalCompletedTrainings = ScormAssignedUser::where('user_email', $user->user_email)
                ->where('completed', 1)->count();
        ...
        // For blue collar users table
         $allBlueCollarUsers = BlueCollarTrainingUser::all();
        foreach ($allBlueCollarUsers as $user) {
            $badge = getMatchingBadge('score', $user->personal_best);
            // This helper function accepts a criteria type and value, and returns the first matching badge
            ...
            $totalCompletedTrainings = BlueCollarTrainingUser::where('user_whatsapp', $user->user_whatsapp)
                ->where('completed', 1)->count();
