...
class ApiCampaignController extends Controller
{
    ...
    public function fetchCampaignDetail(Request $request)
    {
        try {
            ...
            $campaign = Campaign::with(['campLive', 'campLive.campaignActivity', 'trainingAssignedUsers.trainingData', 'scormAssignedUsers.scormTrainingData', 'emailReplies'])->where('campaign_id', $campid)
                ->where('company_id', $companyId)
                ->first();
