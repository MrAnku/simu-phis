...
class PdfController extends Controller
{
    ...
    public function downloadPdf()
    {
        ...
        if (!$id) {
            return back()->with('error', __('Campaign ID not found in session.'));
        }
        ...
        if (!$detail) {
            return back()->with('error', __('Campaign not found.'));
        }
    ...
    public function tprm_full_report()
    {
        ...
        $title = __("Full Report of TPRM Simulation");
        ...
        $Arraydetails[__('Payload Clicked')] = $payloads_clicked_reported ?? 0;
        $Arraydetails[__('Emp Compromised')] = $emp_compromised_reported ?? 0;
        $Arraydetails[__('Training Assigned')] =    $training_assigned ?? 0;
        $ArrayData_labels = [
            ["labels" => __('Payload Clicked')],
            ["labels" => __("Emp Compromised")],
            ["labels" => __("Training Assigned")],
        ];
        $ArrayCount['array_count'] = $training_assigned + $payloads_clicked_reported + $emp_compromised_reported;
        $label = "TPRM";
        return view('tprm-template', compact('Arraydetails', 'camp_live', 'ArrayData_labels', 'ArrayCount', 'label', 'title'));
    }
    ...
    public function tprm_campaigns_wise(Request $request)
    {
        ...
        $companyId = Auth::user()->company_id;
        $company_name = TprmCampaignLive::where('campaign_id', $campId)
            ->where('company_id', $companyId)->first();
        $title = __("Campaign Report of TPRM Simulation") . " - " . $company_name->campaign_name;
    ...
    public function whatsapp_campaigns_wise(Request $request)
    {
        ...
        $company_name = WhatsAppCampaignUser::where('camp_id', $campId)
            ->where('company_id', $companyId)
            ->first();

        // return $company_name;

        $title = __("Campaign Report of WhatsApp Phishing Simulation") . " - " . $company_name->camp_name;
        ...
        return view('whatsapp-template', compact('Arraydetails', 'camp_live', 'ArrayData_labels', 'ArrayCount', 'label', 'title'));
    }
    ...
    public function whatsapp_full_report()
    {
        ...
        $title = __("Full Report of WhatsApp Phishing Simulation");
        ...
        $Arraydetails = [];
        $ArrayCount = [];
        $Arraydetails[__('Link Clicked')] =  $link_clicked;
        $Arraydetails[__('Emp Compromised')] = $emp_compromised;
        $Arraydetails[__('Training Assigned')] = $wtraining_assigned;
        $ArrayCount['array_count'] = $wtraining_assigned + $link_clicked + $emp_compromised;
        // return $Arraydetails;
        $ArrayData_labels = [
            ["labels" => __("Link Clicked")],
            ["labels" => __("Emp Compromised")],
            ["labels" => __("Training Assigned")],
        ];
    ...
    public function  email_full_report()
    {
        ...
        $Arraydetails = [];
        $ArrayCount = [];
        $Arraydetails[__('Payload Clicked')] = $payload_clicked ?? 0;
        $Arraydetails[__('Emp Compromised')] =   $emp_compromised  ?? 0;
        $Arraydetails[__('Training Assigned')] = $training_assigned ?? 0;
    ...
    public function email_campaigns_wise(Request $request)
    {
        ...
        $company_name = CampaignLive::where('campaign_id', $campId)
            ->where('company_id', $companyId)->first();
        // return $company_name;
        $title = __("Campaign Report of Email Phishing Simulation") . " - " . $company_name->campaign_name;
    ...
    public function ai_campaigns_wise(Request $request)
    {
        ...
        $title = __("Campaign Report of AI Vishing Simulation") . " - " . $company_name->campaign_name;
        ...
        $Arraydetails = [];
        $ArrayCount = [];
        $Arraydetails[__('Call Sent')] = $call_send_response_count ?? 0;
        $Arraydetails[__('Call Responded')] =   $call_end_response_count  ?? 0;
        $Arraydetails[__('Traning Assigned')] = $training_assigned ?? 0;
        $ArrayCount['array_count'] = $call_send_response_count + $training_assigned + $call_end_response_count;
        $ArrayData_labels = [
            ["labels" => __("Call Sent")],
            ["labels" => __("Call Responded")],
            ["labels" => __("Training Assigned")],
        ];
    ...
    public function ai_full_report()
    {
        ...
        $call_send_response_count = AiCallCampLive::where('company_id', $companyId)
            ...
            ->count(); // Count those rows
        $title = __("Full Report of AI Vishing Simulation");
