...
class AdminQuishingEmailController extends Controller
{
    public function index(Request $request)
    {
        ...
        $quishingEmails = QshTemplate::with('senderProfile')->get();

        return view('admin.quishingEmails', compact('quishingEmails', 'senderProfiles', 'phishingWebsites'));
    }
    ...
    public function addTemplate(Request $request){
        ...
        return redirect()->route('admin.quishingEmails')->with('success', 'Template added successfully.');

    }

    public function deleteTemplate(Request $request){
        $id = base64_decode($request->id);
        $template = QshTemplate::where('id', $id)->first();
        if ($template) {
            $template->delete();
