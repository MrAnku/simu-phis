...
if (!function_exists('translateHtmlToAmharic')) {
    function translateHtmlToAmharic(string $htmlContent): ?string
    {
        ...
        // Get all text nodes (ignore scripts and styles)
        ...
        foreach ($nodes as $node) {
            ...
            if ($originalText !== '') {
                $messages = [
                    [
                        "role" => "system",
                        "content" => "You are a professional translator. Translate ONLY the text into Amharic. Do not translate HTML entities, and do not add or remove anything else."
                    ],
                    [
                        "role" => "user",
                ...
                try {
                    ...
                    if ($response->successful()) {
                        $translatedText = $response->json()['choices'][0]['message']['content'] ?? $originalText;
        ...
        // Get the updated HTML (remove <!DOCTYPE ...>)
...
if (!function_exists('getScormTraining')) {
    function getScormTraining($id)
    {
        // Validate input
        if (!is_numeric($id) || $id <= 0) {
            return ['status' => 0, 'msg' => 'Invalid SCORM training ID'];
        }
        ...
        try {
            ...
            return ['status' => 0, 'msg' => 'SCORM training not found'];
        }
        ...
        if (empty($s3FilePath) || !Str::endsWith($s3FilePath, '.zip')) {
            return ['status' => 0, 'msg' => 'Invalid or missing ZIP file path'];
        }
        ...
        if (file_exists($publicExtractPath . '/imsmanifest.xml')) {
            return [
                ...
                'scorm_url' => $extractUrl . '/' . $scormTraining->entry_point,
            ];
        }

        // Create extraction directory
        if (!file_exists($publicExtractPath) && !mkdir($publicExtractPath, 0755, true)) {
            return ['status' => 0, 'msg' => 'Failed to create extraction directory'];
        }

        // Create temporary directory
        $tempDir = storage_path('app/temp');
        if (!file_exists($tempDir) && !mkdir($tempDir, 0755, true)) {
            return ['status' => 0, 'msg' => 'Failed to create temporary directory'];
        }

        // Download ZIP file to a temporary path
        $localZipPath = tempnam($tempDir, 'scorm_');
        if ($localZipPath === false) {
            return ['status' => 0, 'msg' => 'Failed to create temporary file'];
        }
        ...
        try {
            ...
            if (file_put_contents($localZipPath, $fileContents) === false) {
                unlink($localZipPath);
                return ['status' => 0, 'msg' => 'Failed to write ZIP file to temporary location'];
            }
            ...
            if ($zip->open($localZipPath) !== true) {
                unlink($localZipPath);
                return ['status' => 0, 'msg' => 'Failed to open ZIP file'];
            }
            ...
            return ['status' => 0, 'msg' => 'Failed to process ZIP file: ' . $e->getMessage()];
        }
        ...
        if (!file_exists($publicExtractPath . '/imsmanifest.xml')) {
            ...
            return ['status' => 0, 'msg' => 'Invalid SCORM package: Missing imsmanifest.xml'];
        }
...
if (!function_exists('atLeastOneUserWithWhatsapp')) {
    function atLeastOneUserWithWhatsapp($groupid, $companyId)
    {
        $hasWhatsapp = false;
...
if (!function_exists('assignGrade')) {
    function assignGrade($rowData, $trainingScore)
    {
        if ($trainingScore >= 90) {
            $rowData->grade = 'A+';
...
if (!function_exists('assignBadge')) {
    function assignBadge($trainingData, $badge)
    {
